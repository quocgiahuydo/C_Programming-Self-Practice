...
void print_linkedlist(node_t *head){ //Print out the value of node 
    ...
    while (temp->next!= NULL){ //Determine when the node end 
        printf("%d -", temp->numb); // Print the value of node based on what is link from head 
        temp = temp->next;
    }
    printf("\n");
}
node_t *create_value(int value, node_t *n){ //Create new value for the node 
    ...
    result->next = n; // determine the next address 
    n->next=NULL;// Ending the next address
